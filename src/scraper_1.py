...
'''
2019 stats
 https://www.pro-football-reference.com/play-index/tgl_finder.cgi?request=1&match=game&year_min=2019&year_max=2019&
 game_type=R&game_num_min=0&game_num_max=99&week_num_min=0&week_num_max=99&temperature_gtlt=lt&c1stat=points&c1comp=
 gte&c2stat=pass_yds&c2comp=gte&c3stat=rush_yds&c3comp=gte&c4stat=turnovers&c4comp=gte&c5stat=fgm&c5comp=gt&c5val=1.0
 &order_by=game_date&offset={}

2018 stats
 https://www.pro-football-reference.com/play-index/tgl_finder.cgi?request=1&match=game&year_min=2018&year_max=2018
 &game_type=R&game_num_min=0&game_num_max=99&week_num_min=0&week_num_max=99&temperature_gtlt=lt&c1stat=points&c1comp=
 gte&c2stat=pass_yds&c2comp=gte&c3stat=rush_yds&c3comp=gte&c4stat=turnovers&c4comp=gte&c5stat=fgm&c5comp=gt&c5val=1.0
 &order_by=game_date&offset={}
...
def scrape_2019_stats():
    '''
    this function pulls statistics for the 2019 NFL regular season, it should return strings, floats, and ints, if not, run the helper functions
    for time of possession and then convert into float functions from lines 127 to 139, the feature engineering function can be found on line 144
    ...
    for i in range(0,600,100):
        ...
        stats_2019.append(pd.read_html(url.format(i))[0])
    df = pd.concat(stats_2019)
    ...
    stats_20191 = []
    for i in range(0,600,100):
        url1 = """https://www.pro-football-reference.com/play-index/tgl_finder.cgi?request=1&match=game&year_min=2019&year_max=2019&game_type=R&game_num_min=0&game_num_max=99&week_num_min=0&week_num_max=99&temperature_gtlt=lt&c1stat=penalties_yds&c1comp=gte&c5stat=fgm&c5comp=gt&c5val=1.0&order_by=game_date&offset={}"""
        stats_20191.append(pd.read_html(url1.format(i))[0])      

    df1 = pd.concat(stats_20191)
    ...
    dfs19 = dfs.merge(dfs1, on=['team1', 'team2','Week'], how='left')
